    ...
    <div x-data="{ ...cadastroPessoaModal(), ...personPhotoModal(), ...consultaPage() }" x-init="init()" style="display:flex;flex-direction:column;gap:16px;">
      ...
      <div class="glass-card" style="padding:16px;border-radius:4px;display:flex;flex-direction:column;gap:12px;">
        ...
        <div x-show="searched && !loadingPessoa && buscouPessoa && pessoasTexto.length === 0 && pessoasFoto.length === 0 && !fotoSearchDone">
          ...
          <button @click="abrirCadastroPessoa(query)"
 ...
 *
 * Gerencia estado de busca por texto, foto, endereço e veículo. O cadastro
 * de nova pessoa (quando a busca não encontra ninguém) é responsabilidade
 * do componente compartilhado cadastroPessoaModal(), mesclado no x-data
 * desta página.
 */
function consultaPage() {
  return {
    ...
    // Cadastro de nova pessoa: estado/métodos vêm de cadastroPessoaModal()
    // (frontend/js/components/cadastro-pessoa-modal.js), mesclado no x-data.
    cpfBuscaErro: "",
